...
public class ExceptionMapper {
    ...
    public static void registerExceptionHandlers(@NotNull JavalinConfig config) {
        ...
        config.routes.exception(AccountFrozenException.class,
                (e, ctx) -> ctx.status(400).json(new ErrorResponse("account_frozen",
                        e.getMessage())));
        config.routes.exception(AccountClosedException.class,
                (e, ctx) -> ctx.status(400).json(new ErrorResponse("account_closed",
                        e.getMessage())));
        config.routes.exception(InvalidAmountException.class,
                (e, ctx) -> ctx.status(400).json(new ErrorResponse("invalid_amount",
                        e.getMessage())));
        config.routes.exception(InsufficientFundsException.class,
                (e, ctx) -> ctx.status(400).json(new ErrorResponse("insufficient_funds",
                        e.getMessage())));
        config.routes.exception(WithdrawalLimitExceededException.class,
                (e, ctx) -> ctx.status(400).json(new ErrorResponse(
        ...
        config.routes.exception(InvalidRequestException.class,
                (e, ctx) -> ctx.status(400).json(new ErrorResponse("invalid_request",
                                e.getMessage())));
        ...
        config.routes.exception(com.google.gson.JsonParseException.class,
                (e, ctx) -> ctx.status(400).json(new ErrorResponse("bad_request",
                        "Malformed request body")));

        // Not found → 404
        config.routes.exception(AccountNotFoundException.class,
                (e, ctx) -> ctx.status(404).json(new ErrorResponse(
                        "account_not_found", e.getMessage())));
        config.routes.exception(UserNotFoundException.class,
                (e, ctx) -> ctx.status(404).json(new ErrorResponse("user_not_found",
                        e.getMessage())));
        ...
        config.routes.exception(UnauthorizedException.class,
                (e, ctx) -> ctx.status(401).json(new ErrorResponse("unauthorized",
                        e.getMessage())));
        config.routes.exception(ForbiddenException.class,
                (e, ctx) -> ctx.status(403).json(new ErrorResponse("forbidden", e.getMessage())));
        config.routes.exception(AccountLockedException.class,
                (e, ctx) -> ctx.status(423).json(new ErrorResponse("account_locked",
                        e.getMessage())));  // 423 Locked
